...
def init_db_concurrency():
    ...
    try:
        ...
        # Keep init best-effort but expose errors if needed
        pass
            ...
            with db_lock:
                ...
                if row:
                    ...
                    if random.choice([True, False]):
                        ...
                        cursor.execute(
                            "UPDATE account_state SET balance = ?, peak_balance = ? WHERE id = 1;",
                            (str(new_balance), str(new_peak)),
                        )
                        cursor.execute(
                            "INSERT INTO transactions (timestamp, type, amount, balance) VALUES (datetime('now','localtime'), 'MARKET_GAIN', ?, ?);",
                            (str(gain), str(new_balance)),
                        )
                    else:
                        ...
                        cursor.execute(
                            "UPDATE account_state SET balance = ? WHERE id = 1;",
                            (str(new_balance),),
                        )
                        cursor.execute(
                            "INSERT INTO transactions (timestamp, type, amount, balance) VALUES (datetime('now','localtime'), 'MARKET_LOSS', ?, ?);",
                            (str(-loss), str(new_balance)),
                        )

                    conn.commit()
                conn.close()
        except Exception:
            # Don't crash the loop; keep running and retrying
            pass
        ...
        if choice == "1":
            ...
            try:
                ...
                with db_lock:
                    conn = sqlite3.connect('trading_system.db', timeout=15.0)
                    cursor = conn.cursor()
                    # Update balance and ensure peak_balance reflects the new total
                    cursor.execute(
                        "UPDATE account_state SET balance = balance + ?, peak_balance = max(peak_balance, balance + ?) WHERE id = 1;",
                        (str(val), str(val)),
                    )
                    cursor.execute(
                        "INSERT INTO transactions (timestamp, type, amount, balance) VALUES (datetime('now','localtime'), 'DEPOSIT', ?, (SELECT balance FROM account_state WHERE id=1));",
                        (str(val),),
                    )
                    conn.commit()
                    conn.close()
            ...
            if surplus > 0:
                ...
                if confirm == 'y':
                    with db_lock:
                        ...
                        cursor = conn.cursor()
                        cursor.execute(
                            "UPDATE account_state SET balance = balance - ? WHERE id = 1;",
                            (str(surplus),),
                        )
                        cursor.execute(
                            "INSERT INTO transactions (timestamp, type, amount, balance) VALUES (datetime('now','localtime'), 'WITHDRAW_SUCCESS', ?, (SELECT balance FROM account_state WHERE id=1));",
                            (str(-surplus),),
                        )
                        conn.commit()
                        conn.close()
            ...
            with db_lock:
                try:
                    ...
                    cursor = conn.cursor()
                    cursor.execute(
                        "SELECT timestamp, type, amount, balance FROM transactions ORDER BY timestamp DESC LIMIT 10;",
                    )
                    rows = cursor.fetchall()
                    conn.close()
                    for r in rows:
                        # amount and balance may be stored as text; cast to float for display
